...
template<class T> void SegmentTree<T>::update(int indx , T val) {
    int segTreeIndx = (segTreeSize/2) + indx ;
    tree[segTreeIdx] = val ;
    while(parent(segTreeIndx) >= 0) {
        segTreeIndx = parent(segTreeIndx) ;
        if(right(segTreeIndx) < segTreeSize) {
            tree[segTreeIndx] = combine(tree[left(segTreeIndx)] , tree[right(segTreeIndx)]) ;
        }
        if(segTreeIndx == 0) {
            break ;
        }
